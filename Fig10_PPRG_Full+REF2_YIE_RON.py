### Fig9: PPRG-Full + REF2 15% (when hits ARO at 25%)
...
ax2.set_ylabel('RON (---)', fontsize=12)  # we already handled the x-label with ax1
...
MTBE_in_Gasoline = 0/100  ; ETH_in_Gasoline  = 0/100 #10%
REF2_in_Gasoline = 15/100 #10%
...
ax1.plot(SC,gasolineSC,color='red', label='Full PPRG + REF2 15%', marker='x')
ax1.plot(SC,gasolineSCARO,color='green',label='ARO', marker='o')
ax2.plot(SC,gasolineSCRON,color='blue',label='Full PPRG + REF2 15%',linestyle='dashed', marker='x')
...
#ax1.legend()
#ax2.legend()
text = ax1.text(-1.385,45.3, "|", fontsize=13)
