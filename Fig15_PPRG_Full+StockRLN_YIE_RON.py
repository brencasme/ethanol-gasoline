## Fig14: PPRG-Full + stock_RLN (60%) (when hits ARO at 25%) : stock remaing LN (considering 30% divert to ISO)
...
ax1.plot(SC,gasolineSCARO,color='green',label='ARO in Full PPRG')

#ETH = True

# Supply Chain (SC) or Exogenous Variables

MTBE_in_Gasoline = 0/100  ; ETH_in_Gasoline  = 0/100
...
ax1.plot(SC,gasolineSC,color='red', label='Full PPRG + 60% StockRLN', marker='x')
ax2.plot(SC,gasolineSCRON,color='blue',label='Full PPRG + 60% StockRLN',linestyle='dashed', marker='x')
ax1.plot(SC,gasolineSCARO,color='green',label='ARO in Full PPRG + 60% StockRLN', marker='x')
...
#ax1.legend()
#ax2.legend()
text = ax1.text(-1.855,39.3, "|", fontsize=13)
